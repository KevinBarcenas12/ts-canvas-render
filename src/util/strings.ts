...
export function isValidVideoExtension(string: string): [ boolean, CodecType ] {
    for (let i = 0; i < VIDEO_EXTENSIONS.length; i++) {
        if (string.endsWith(VIDEO_EXTENSIONS[i][0])) {
            return [true, VIDEO_EXTENSIONS[i][1]];
        }
    }
    return [false, null];
}
...
const NormalFormat: NormalFormatObj = {
    withHours: '$h:$m:$s',
    default: '$m:$s',
    withZero: false,
...
export function getTimeInFormat(
    time: number,
    formatObj: Partial<NormalFormatObj> = NormalFormat
): string {
    const isReversed = time < 0;
    if (isReversed) time = -time;
    const format = { ...NormalFormat, ...formatObj, };
...
const ExtendedFormat: ExtendedFormatObj = {
    symbols: {
        negative: '-',
        positive: '+',
    },
...
export function getTimeExtended(
    time: number,
    formatObj: Partial<ExtendedFormatObj> = ExtendedFormat
): string {
    if (time < 0) time = -time;
    const format = { ...ExtendedFormat, ...formatObj, };

    const Format = time >= 3600
    ...
    const symbol = time < 0 ? format.symbols.negative : format.symbols.positive;

    return Format
        .replace(format.placeholders.symbol, symbol)
